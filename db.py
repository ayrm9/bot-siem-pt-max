...
class DbVariable:
    ...
    def set(self, value):
        self.__value = value
        try:
            execute_with_values(self.db_connection, self.db_cursor, self.db_query_set, self.__value)
...
class DbTable:
    ...
    def append(self, value):
        ...
        try:
            execute_with_values(self.db_connection, self.db_cursor, self.db_query_set, value)
            return True
        except sqlite3.Error as ex:
...
def execute_with_values(connect: sqlite3.Connection, cursor: sqlite3.Cursor, query: str, *variables):
    ...
    result = cursor.fetchall()
    connect.commit()
    log("db.execute_with_values: \nquery - {0} {1}. \nresult - {2}".format(query, variables, result))
    return result
