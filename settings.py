...
# MAX выпускает сертификат российским УЦ (Минцифры), которого нет в наборе certifi,
# поэтому обычно нужно указать путь к системному хранилищу сертификатов, например:
#   max_verify_ssl = "/etc/pki/tls/certs/ca-bundle.crt"     # RED OS, RHEL, CentOS
#   max_verify_ssl = "/etc/ssl/certs/ca-certificates.crt"   # Debian, Ubuntu, Astra
# False отключает проверку целиком - крайняя мера, так делать не рекомендуется.
max_verify_ssl = True
...
# (одинаковые события схлопываются в одну строку со счетчиком и считаются за одну)
max_events_count = 5

# Максимальная длина описания одного события, длиннее - обрезается
max_event_length = 400

# Таймаут запросов к SIEM в секундах
siem_timeout = 60

# Сколько дней помнить, в какие чаты уходил инцидент.
# Нужно, чтобы отметку «кто подтвердил/закрыл» дописать во все чаты сразу.
incident_messages_keep_days = 30
